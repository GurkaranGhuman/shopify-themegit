...
$(this).parents('.customizer_wrap').find('.variantsshippingtext').text(defaultshiptext);
}
    else{
      $(this).parents('.customizer_wrap').find('.variantsshippingtext').text(metatext);
    }
});
...
var initials = {
  ...
  if($('.initials').hasClass('phone-case-bundle') || $('.initials').hasClass('notebook') ) {
   ...
   if($('.initials').hasClass('phone-case') || $('.initials').hasClass('laptop-case') || $('.initials').hasClass('notebook') || $('.initials').hasClass('airpods-belt') || $('.initials').hasClass('card-phone-case') || $('.initials').hasClass('sling-phone-case')  || $('.initials').hasClass('airpods-pro') || $('.initials').hasClass('keychain') || $('.initials').hasClass('phone-case-bundle') )
   { 
      if(!phone_case){
    ...
    else if($('.initials').hasClass('airpods') || $('.initials').hasClass('airpods-sling-case') || $('.initials').hasClass('keybell-keychain') ) {
      return validateShortMonogram2(localStorage.getItem('mdsEngraving') != null ? localStorage.getItem('mdsEngraving') : '');       
    }     
      ...
      if(airpods_mono=="shortMonogram2" ) {
        ...
        return localStorage.getItem('mdsEngraving') != null ? localStorage.getItem('mdsEngraving').substr(0, 4) : ''; 
        }
      }
    }
...
var foil = {
  load: function(clickTarget) {
    $('.engrave:not(.cart-engrave), .zodiac-engrave:not(.cart-engrave)').removeClass('gold').removeClass('silver').addClass(this.get().toLowerCase());
...
var customizer = {
    ...
    if(artwork.get() != "No Artwork" || zodiac.get() != "No Zodiac") {
     ...
     if(initials.get('null',true) != "") {
      ...
      if(mono_gra.indexOf(searchTerm) != -1){
        var final_mono_gra = mono_gra.replace(searchTerm, '<img style="width: 15px;" src="https://cdn.shopify.com/s/files/1/0242/7060/2321/files/Bunny-finalversion.png?v=1673421947">');
      }
       else {
    ...
    text: function(hasError) {
      if(hasError) {
        ...
        if($('.initials').hasClass('shortMonogram')) {
           ...
           else if($('.initials').hasClass('card-holder') || $('.initials').hasClass('zipped-wallet') || $('.initials').hasClass('card-case')|| $('.initials').hasClass('magsafe-wallet')|| $('.initials').hasClass('passport-holder')) {
            $('.initials_text .error').text('Maximum 4 Characters with only English letters, numbers and the symbols: . & and ♡ (no spaces)');
          }
          else if($('.initials').hasClass('airpods') || $('.initials').hasClass('airpods-sling-case') || $('.initials').hasClass('keybell-keychain')) {
  ...
  change: {
    ...
    text: function(e) {
      ...
      if($('.initials').hasClass('airpods') || $('.initials').hasClass('airpods-sling-case') || $('.initials').hasClass('keybell-keychain') ) { //for airpods
       for (var position = 0; position < text.length; position++) {
      	if (text.charAt(position) == '.') {
          limit = 5; //for airpods
          dotcounter++;
        ...
        if(dotcounter==0) {
          ...
          $('.initials.keybell-keychain').attr('maxlength',4);
        }
        else {
      ...
      if($('.initials').hasClass('card-holder') || $('.initials').hasClass('zipped-wallet') || $('.initials').hasClass('card-case')|| $('.initials').hasClass('magsafe-wallet')|| $('.initials').hasClass('passport-holder')) { //for airpods
       for (var position = 0; position < text.length; position++) {
      	if (text.charAt(position) == '.') {
          limit = 6; //for airpods
          dotcounter++;
    ...
    if(text.length <= limit) {
        if(text.length <= (limit-1) && (regex.test(text) || text == '' || is_rabbit ) && (limit== 6 || (limit < 6) ) ) {  //for airpods
...
let phone_cases_qty = 0;
 ...
 if(cart.items[x].variant_options[0].includes('Emerald')){ 
      ...
      if(cart.items[x].handle.includes('easy-apply-screen-protector-1')) {
       ...
       if(cart.items[x].product_type == 'Phone Case' || cart.items[x].product_type == 'Card Phone Case' || cart.items[x].product_type == 'Sling Phone Case' || cart.items[x].product_type == 'Silicone Phone Case'){
         phone_case = true;
        }
