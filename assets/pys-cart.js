...
$(document).click(function(event){
  $(".product-form__error-message-wrapper.product-form__main-error-message").hide();
  $(".product-form__error-message-wrapper.product-form__sticky-error-message").hide();
});  
    ...
    function addToCart(variant_id, qty, props = null, ButtonName) {
        ...
        .done(function() {
          ...
          .fail(function(XMLHttpRequest) {
              ...
              {
                $(".product-form__error-message-wrapper.product-form__main-error-message").show();
                $(".product-form__error-message-wrapper.product-form__main-error-message .product-form__error-message").text(response.description);
              }
              if(ButtonName == 'sticky-atc')
              {
                $(".product-form__error-message-wrapper.product-form__sticky-error-message").show();
                $(".product-form__error-message-wrapper.product-form__sticky-error-message .product-form__error-message").text(response.description);
              }
          });
    }
...
$(document).on('click', '.cart_add_btn', function(e) {
    ...
    if(variant_id == 40353994113105 || variant_id == 40315459895377 || variant_id == 40266306224209 || Ptext == '' || Pcolor == '' || variant_id == 40329240936529)
    {    
    var props = {
        'Text': '',
        'Foil Colour': ''
...
$(document).on('click', '.floating_atc_button', function(e) {
    ...
    if(variant_id == 40315459895377 || variant_id == 40266306224209 || Ptext == '' || Pcolor == '')
    {    
    var props = {
        'Text': '',
        'Foil Colour': ''
...
$(document).on('click', '.free_gift_product_aus', function(e) {
    ...
    $(document).on('click', '.cart_add_btn_gift_cart', function(e) {
        ...
        var variant_id = $(this).attr('data-id');
        var ButtonName = '';
        if(variant_id == 40315459895377 || variant_id == 40255413420113)
        {    
        var props = {
            'Text': '',
            'Foil Colour': ''
...
$('.color-swatch').click(function(){
    ...
    $('body').find('.floating_atc_button .custom_atc_text_name').text("Join the Waitlist");
    },500);
    }
    else{
    setTimeout(function(){
       $('body').find('.floating_atc_button .custom_atc_text_name').text("Add To Bag");
  ...
  // ends
    ...
    function addMatchingItem(btn, variant, is_art_series, is_zodiac_series, is_upsell, productType, preorder_text) {
        btn.addEventListener('click', function(e) {
            ...
            if ($(btn).hasClass('upsell_item--add') || $('.slider_matching--items').hasClass('screen-protector') || $('.slider_matching--items').hasClass('leather-protectant')) {
                props = {
                    'Text': '',
                    'Foil Color': ''
                };
 ...
 function getMatchingItems(container='', cart) {
      ...
      $.getJSON('/collections/'+collectionHandle+'/products.json', function(products) {
        for (let index = 0; index < products['products'].length; index++) {
          ...
          if(products['products'][index].id == '7089160454225')
          {
               ...
               if(cart.items[i].id == 40315459895377){
                 is_mini_tote_free_gift = true;
                  ...
                  if(cart.items[i].product_title.includes('Phone')){
                   ...
                   else if ((final_total >= 110) && (final_total < 119)) {
                        $('.progress-bar__fillforfreegift').css('width', '99%');
                    }
                  
                  else if ((final_total >= 120)) {
                 ...
                 if(variant_title_new == "Gift Keychain"){
                  ...
                  cart_item_div_new += "<a><p>" + SplitTitle[0] + "</p></a><span class='ajaxcart__product-meta minishoppergift'>" + ColNAme + "</span>"
                   ...
                   if(variant_title_new == "Gift Keychain"){
                    cart_item_div_new += "<div class='wc_qty_box quantity-box'>" 
                     cart_item_div_new += "<ul class='QuantityBox cart_p_quantity'><li class=''>"
